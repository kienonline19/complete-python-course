...
def InNoiDungFile_CSV(filename):
    ...
    print(template2.format('TT', '\ufeffTên khu vực', ' |Tên Tỉnh-Thành phố', '  |Dân số', '  |Diện tích',
                           ' |Có biên giới với TQ', ' |Có biên giới với Lào', 'Có biên giới với Campuchia',
                           'Có giáp với biển'))
    ...
    for kv in data:
        ...
        if (co_bg_vs_tq == co_bg_vs_lao and co_bg_vs_lao == keyword) or \
                ...
                (co_giap_bien == keyword and any(
                    x == keyword for x in (co_bg_vs_tq, co_bg_vs_lao, co_bg_vs_campuchia))):
            dem += 1
